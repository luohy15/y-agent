...
def _handle_scheduled_action(action: str, event: dict) -> dict:
    ...
    if action == "fetch_rss_xml":
        from worker.steps.fetch_rss_xml import handle_fetch_rss_xml
        return asyncio.run(handle_fetch_rss_xml())
    ...
    if action == "sync_finance":
        from worker.steps.sync_finance import handle_sync_finance
        return asyncio.run(handle_sync_finance())
