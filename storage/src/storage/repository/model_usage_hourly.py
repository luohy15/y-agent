"""Function-based model_usage_hourly repository (idempotent hourly upsert)."""
...
from storage.dto.model_usage_hourly import ModelUsageHourly
from storage.entity.model_usage_hourly import ModelUsageHourlyEntity
from storage.util import get_utc_iso8601_timestamp


def _entity_to_dto(entity: ModelUsageHourlyEntity) -> ModelUsageHourly:
    return ModelUsageHourly(
        id=entity.id,
        user_id=entity.user_id,
        usage_date=str(entity.usage_date),
        usage_hour=int(entity.usage_hour),
...
def upsert_hourly(user_id: int, rows: list[dict], synced_at: str) -> int:
    """Upsert usage rows on (user, date, hour, source, scope_id, model).
    Re-pulling the still-mutating current hour overwrites; a finalized past
    hour is a no-op."""
    with get_db() as session:
        for row in rows:
            stmt = insert(ModelUsageHourlyEntity).values(**_values(user_id, row, synced_at))
            update_values = {
            ...
            stmt = stmt.on_conflict_do_update(
                constraint="uq_model_usage_hourly",
                set_=update_values,
            )
...
def list_for(
    ...
    with get_db() as session:
        ...
        rows = (
            query.order_by(
                ModelUsageHourlyEntity.usage_date.desc(),
                ModelUsageHourlyEntity.usage_hour.desc(),
                ModelUsageHourlyEntity.source.asc(),
                ModelUsageHourlyEntity.model.asc(),
            )
            .limit(limit)
            .all()
        )
