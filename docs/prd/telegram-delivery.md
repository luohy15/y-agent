...
| 3468 | Implemented the manager-only gate (worker eligibility, immediate-attachment API path, `deliver_death` topic branch), private-only inbound webhook gate with forum discovery/`/clear`-branch/thread-plumbing removal, and full `tg_topic` API/service/repository/entity/DTO removal and `y telegram send --topic` retirement; routine-caller migration (sub-task 6) delivered separately | - | `pages/plan-3460-telegram-group-removal.md` | - | - | implemented, pending review |
