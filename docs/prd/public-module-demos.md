...
|------|---------|--------|------|-----------|--------|--------|
| 3158 | Shipped **three** of the four planned demos: stable unauthenticated read-only `/demo/chat`, `/demo/todo`, `/demo/note` backed by the active production module UI and deterministic fictional fixtures. Host half on y-agent `17aafea` (`bd3313e` public-aware loader + mandatory `demo` export; `1e80561` public demo projection + bundle gate with exact GET-scoped auth exceptions; `af23ceb` restricted fail-closed demo runtime + routes; `25b1617` docs launch links; `d0910c5` removal of the old `/showcase` screenshot pipeline; `9e9f45c` boundary suite + e2e config; `17aafea` `ui_public` contract statements). Module half on y-module `5ad0b11` (`0496554` chat demo via shared `ConversationView`; `31a5941` note demo via shared list presentation; `e289aa0` todo demo via shared table/kanban leaves; `5ad0b11` fictional trace id + three-demo copy). **Link deferred** at plan sub-task M0 after 3 review rounds / 15 blocking findings: it is the only showcase that is not already a module, and the shared-renderer fix reached into the shipped `file` module and the `.sdk` build contract. Its Getting Started screenshot is retained. **Deferred residual risk:** demo isolation is in-realm only; module code that creates a same-origin iframe can still reach a logged-in visitor's JWT, cookie, native fetch and IndexedDB (reproduced in a browser). Accepted because module bundles are first-party and hash-verified; pinned by a KNOWN GAP test that flips when closed. **Not released:** nothing pushed, published, repointed or deployed. | - | `pages/plan-3158-public-module-demos.md` | `pages/decision-3042-public-dispatch-scope.md` | `pages/review-3158-public-module-projection.md`, `pages/review-3158-demo-loader.md`, `pages/review-3158-demo-runtime.md`, `pages/review-3158-chat-demo.md`, `pages/review-3158-todo-demo-adapter.md`, `pages/review-3158-note-demo-adapter.md`, `pages/review-3158-link-module-authority.md`, `pages/review-3158-security-suite.md`, `pages/review-3158-public-docs.md`, `pages/review-3158-showcase-removal.md`, `pages/review-3158-release-readiness.md` | built and audited; awaiting Roy's release approval |
