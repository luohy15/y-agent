...
| 2989 | The displayed context window is **not** a y-agent lookup: it is Claude Code's own `result.modelUsage[*].contextWindow`, which `monitor.py` copies into `Chat.context_window` verbatim and the UI only formats. A `sol` chat reported 200K because the bot was configured as plain `gpt-5.6-sol`, and Claude Code treats an unknown custom model as a 200K variant; the working 1M bots use the client's `[1m]` suffix convention (`opus[1m]`, `sonnet[1m]`). Fix is configuration plus a relay companion change, not code: `sol` becomes `gpt-5.6-sol[1m]`, and claude-relay-service strips the client-only suffix before upstream dispatch (see the CRS bridge feature note, commit `4527d56a`). y-agent's own diff is **test-only** (`worker/tests/test_claude_code_runner.py`, commit `cdf1725`) proving the launcher forwards the suffixed model verbatim and sets `_CLAUDE_CODE_ASSUME_FIRST_PARTY_BASE_URL=1` only when a relay `base_url` exists. Deliberately rejected: any server-side model→window map or UI fallback, which would let the badge claim 1M while the client still compacts at 200K. Historical chats (e.g. `68c801`) keep their real 200K telemetry | - | `pages/plan-2989.md` | - | `pages/review-2989-sol-1m-relay-normalization.md` | shipped; fresh chat persists `context_window=1000000` |
