...
| 3046 | Right drawer is a four-category switcher (Chat, Notes, Files, Diff) whose controls sit in a horizontal header inside the resizable pane; the split bottom-right Context area is removed. Chat is the module entry `artifact:chat` mounted through the existing module list, `artifact:<slug>` keys, loader, and `ArtifactMount` path, removing the post-3042 duplication with host `ChatList`, which stays only for the logged-out public trace projection. Links is a left-sidebar category only; a persisted right key of `links` migrates to `notes`. Establishes that `panel` is a location-independent narrow-column contract: one published surface can mount in either sidebar, and **contract v6** adds `usePanelLocation(): "left" \| "right"`, provided by `ArtifactMount` for `surface="panel"` only and defaulting to `"left"`, so `detail`/`shell` and older call sites are unaffected. The chat module (v7) consumes it to apply the host's selected-intent `traceId` only at the right mount, latching the last applied host value so a same-value republish on `chat.open` cannot clobber a user-typed filter while a genuine host clear still unfilters. Accepted regression: no authenticated chat-list fallback, so disabling chat or rolling it back to a version without a `panel` drops the Chat entry from the right drawer, matching existing left-sidebar behavior (`ChatFallbackView` guards the conversational shell, not browsing). Deferred by explicit waiver: no vitest/jsdom harness exists under `modules/`, so M1's render-both-locations React test is replaced by `modules/chat/tests/test_panel_trace_filter.mjs`, which bundles the real `panel.tsx` and covers the helpers but not the effect wiring. No new module surface, no second loader, no design phase. | - | `pages/plan-3046-right-sidebar.md` | - | `pages/review-3046-right-sidebar.md`, `pages/review-3046-chat-panel-trace-filter.md` | shipped |
| 3068 | **Contract v6 → v7**: `@y/host` exports the host's `CodeEditor` (CodeMirror with host-lazy language grammars) as a versioned leaf, the second growth of the render-leaf set after todo 3042's `ArtifactView`/`PatchDiff`/`ImageLightbox`/PNG-capture. Bundling CodeMirror directly into a module measured ~957 KB unminified (core + one language) against the 250 KB module ceiling — over ceiling even minified and bare (~267 KB) — so the file module's detail workspace (`ui:file`) imports the host leaf instead of vendoring the editor. `usePanelLocation()` (v6) gets its first non-chat consumer: the Files `panel` mounts in either sidebar and resolves per-location VM/work-directory context through the existing intent channel. No new UI surface; `file` claims `panel` + `detail` only. Full requirements and the module-side boundary (the file module owns no database tables; it is a control-plane module over host VM infrastructure) live in `docs/prd/module-system.md`. | - | `pages/plan-3068-file-module.md` | `pages/decision-3068-codemirror-bundle-measurement.md` | `pages/review-3068-codeeditor-host-export.md`, `pages/review-3068-file-browser-seam.md`, `pages/review-3068-file-panel.md`, `pages/review-3068-file-detail-workspace.md` | shipped |
