...
| 3046 | Right sidebar is an activity-bar-style, full-height panel switcher that mounts module `panel` surfaces through the existing module list, `artifact:<slug>` keys, loader, and `ArtifactMount` path; the split bottom-right Context area is removed. Confirms `panel` is a location-independent narrow-column contract rather than a left-sidebar-only surface, so one published surface now mounts in either sidebar. Host built-ins reduce to four (notes / links / files / diff): Chat becomes the module entry `artifact:chat`, removing the post-3042 duplication with host `ChatList`, which stays only for the logged-out public trace projection. Accepted regression: no authenticated chat-list fallback, so disabling chat or rolling it back to a version without a `panel` drops the Chat entry from the right rail, matching existing left-sidebar behavior (`ChatFallbackView` guards the conversational shell, not browsing). No new module surface, no second loader, no design phase. | - | `pages/plan-3046-right-sidebar.md` | - | `pages/review-3046-right-sidebar.md` | shipped |
