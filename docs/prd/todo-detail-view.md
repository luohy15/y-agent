...
- **Redesigning the todo detail.** This feature relocates and unifies an existing
  ...
  design phase.
- **Redesigning the Todo list.** Table and kanban modes, their filters, and
  sorting are unchanged apart from what the detail round trip requires. The
  module-owned detail activity rail is removed in todo 3582; stored history
...
| 3187 | Compact todo-ID input inside the in-place detail so the user can switch directly to another todo without returning to the retained list. Module-only change in y-module `todo/ui`; no host or contract change. No design phase: a small input affordance inside an existing header, not a new screen or restyle. | - | `pages/plan-3187-todo-detail-id-input.md` | Accept both a bare positive-decimal todo id and a pasted trace URL (relative `/trace/<id>` or absolute http(s) with exactly that pathname), rejecting query/fragment/other paths client-side without an API call; route every switch through the existing `openTodoDetail` authority rather than adding a second state writer, so the dirty-discard prompt gates a switch exactly as it gates Back and a host intent; keep the consume-once intent nonce host-intent-only; split unavailable handling by provenance so a mount-restored stale id still self-heals to the list while an interactively entered missing id stays in the unavailable state with a path back; solved inside the module with no browser-contract bump (`min_host_version` stays 10) | `pages/review-3187-todo-detail-id-input.md` | shipped: y-module main `453bb00`, todo module v11 (ui `b0277a9f9f8a`), published 2026-08-18. `min_host_version` stays 10, so no host deploy is required. Rollback: `y module activate todo 10`. User verification of the interactive cases (V1) still pending |
| 3530 | Preserve the Todo list's loaded pages and scroll position when an item status changes. Root cause was the host `optimisticListMutate` helper writing an explicit `undefined` into every matching SWR key, which emptied the rendered list and collapsed the scroll container; the Todo module then refetched only page 1 and layered duplicate action-level refreshes on top. Spans y-agent `web/src/utils/optimisticMutate.ts` and y-module `todo/ui`. No design phase: no visible layout change. | - | `pages/plan-3530-todo-list-scroll.md` | Fix the helper at its authority with a one-argument exact-key `mutate(key)` (revalidate without clearing data) instead of copying it into the module; pass `revalidateAll: true` to the panel and table infinite lists so a write refetches every loaded page; make the host helper the sole reconciliation owner by removing the duplicate `onAction` / `onMoved` / `onRevalidate` callbacks; make the detail view's global revalidate non-destructive with `populateCache: false`; rely on browser scroll anchoring and natural clamping rather than any persisted pixel offset | `pages/review-3530-host-optimistic-mutate.md`, `pages/review-3530-todo-module-refresh-ownership.md` | pending publication authorization: host candidate `7e3dcc8` (plus this docs row) to y-agent main, todo module candidate `d241aac` on y-module main to be published as v22. No contract bump (`min_host_version` unchanged). Rollback: revert the host commit and redeploy, and/or `y module activate todo 21` |
| 3582 | Remove the right-side Activity panel from the Todo module's detail surface, together with client code that becomes dead with it, while preserving stored history and the host History section. Add a Tag-style in-place refresh at the far right of the Todo list's mode/status filter row (the list shown after Back) so the list revalidates without losing mode, filters, selection, or loaded pages. Module-scoped work in y-module `todo/ui` plus this y-agent docs-only update. No design phase: copies Tag detail and drops an existing rail. Placement of that list refresh is superseded by todo 3674, which replaces it with the host centre-tab chrome's generic refresh (revalidate every key the tab subscribes to, then remount that tab); the list's own revalidation semantics are a subset of that and are unchanged. | - | `pages/plan-3582-todo-detail-activity-removal-refresh.md` | Module-only; no host code or contract bump. Round 1 placed the button in the detail header strip mutating the two exact `TraceView` SWR keys; round 2 (user clarification) moved it to the list filter row, reusing the existing non-destructive `/api/todo/` revalidation and the table `revalidateAll` path, and removed the detail button and its helper. Copies Tag detail (600 ms spin plus unmount cleanup). Handler writes no selection, filter, sort, or navigation preference. | `pages/review-3582-todo-detail-activity-refresh.md` | published: todo module v25 active (round 1 `1776d01` as v24, round 2 `5807279` as v25, ui sha256 `5e27ca64cd06…`, `min_host_version` 13) on y-module main; y-agent docs-only (this row) on y-agent main, no push. Rollback target v24. No contract bump. Rollback: `y module activate todo 23`. List-refresh placement superseded by 3674. |
