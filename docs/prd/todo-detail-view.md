...
| 3582 | Remove the right-side Activity panel from the Todo module's detail surface, together with client code that becomes dead with it, while preserving stored history and the host History section. Add a Tag-style in-place refresh on Todo detail so the open todo and its related lists reload without losing selection, filters, navigation state, or unsaved edits. Module-scoped work in y-module `todo/ui` plus this y-agent docs-only update. No design phase: copies Tag detail and drops an existing rail. | - | `pages/plan-3582-todo-detail-activity-removal-refresh.md` | Module-only; no host code or contract bump. Refresh mutates the two exact `TraceView` SWR keys rather than a substring predicate or a contract-exported handle. Button lives in the module-owned header strip after Open, copying Tag detail (600 ms spin plus unmount cleanup). Hidden list is not refreshed. Unsaved edits survive because `TraceTodoDetail` resets its patch only on `todo_id` change. | `pages/review-3582-todo-detail-activity-refresh.md` | published: todo module v24 active (candidate `1776d01` landed on y-module main, ui sha256 `8f7e0b072da5…`, `min_host_version` 13); y-agent docs-only (this row) on y-agent main, no push. No contract bump. Rollback: `y module activate todo 23` |
