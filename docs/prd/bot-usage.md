...
| 2980 | Daily-tokens heatmap: the no-usage level was `SOL.base02`, the same value as the card border and table header, so empty cells vanished into the chrome under solarized-light. All five levels are now one alpha ramp of Solarized green (`rgba(133,153,0,0.12/0.30/0.55/0.78/1)`), identical in both themes | - | - | - | - | shipped (`bot` artifact v3, `186de92dc666…`) |
