...
| 3527 | Enabled the trace liveness watchdog backstop (research 3526 P1), then made the trace-level awaiting write mechanism-enforced rather than a per-skill prose duty (P3). Phase 1: re-verified the classifier gates and orphan handling on current `main` before flipping, then removed `Enabled: false` from `CheckTraceLivenessSchedule` so the rule defaults on like its eight siblings. Phase 2: one canonical "机器等待与看门狗" paragraph in `AGENTS.md` names the live schedule and defines machine evidence once; the forbidden-third-ending paragraph, the tmux evidence bullet and the `y chat` reopen paragraph became one-sentence pointers | - | `pages/plan-3527-watchdog-enable.md` | - | `pages/review-3527-watchdog-enable.md` | host `312c47e` deployed via Actions run 34731692058 (`deploy-web.yml` correctly did not run); EventBridge `y-agent-WorkerFunctionCheckTraceLivenessSchedule-j8RQKNokrsLd` verified `ENABLED` in us-east-1. First observed pass 2026-09-13 09:57:08 UTC+8: `status=ok scanned=6 candidates=1 claimed=1 suppressed=0 failed=0`, one claim rather than the predicted first-tick burst. No watchdog code, notice wording or awaiting-contract change; rollback is a code-only revert plus disabling the rule first, no schema change. Config delivery: `pages/hr-3527-watchdog-config.md`, `AGENTS.md` 52,286 B → 51,006 B (**-1,280 B**, floor was 800 B), all 47 `SKILL.md` byte-identical (SHA-256 verified), awaiting-write skill set unchanged at `api-latency-review`/`dev`/`impl`/`review`; uncommitted on home-repo main pending Roy's commit decision. Reported not designed around: untracked tmux panes have no grace, the notice stays fault-shaped, and the pass summary logs counts but no todo ids so the single claim could not be attributed from logs. S11 resolved negative: todo 3523 was not deployed, so the context-note requirement was deliberately omitted; 3504/3517/3523 untouched |
| 3523 | Optional factual attention-DM context via awaiting-only --note; validated writer summaries and sanitized fault tails; entry-only notice contract retained | - | `pages/plan-3523-awaiting-notice-context.md` | - | `pages/review-3523-awaiting-notice-context.md` | Host `773287a` deployed successfully via Actions 34732294271; deployment: `pages/deploy-3523-awaiting-notice-context.md`. Safe canonical CLI sync verified --note support; shared and skill callers adopted locally, uncommitted and unpublished: `pages/impl-3523-caller-adoption.md`. No migration, production mutation smoke, or browser checks. |
