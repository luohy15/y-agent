...
Chat is the **third full-stack module** (`$Y_AGENT_HOME/modules/chat`, active v12
...
**v5**, `ui=e859bebc25dc` / `api=628244cb50ae`). **v1–v4 all hang on ordinary-file
...
(v1/v2 never called a route the cut deleted, but that distinction is moot while
they carry the hang; rolling back also drops the three M5 features restored on
top of the pre-cut pair: Import Note, the History link, and the
sandboxed-preview keyboard bridge). It is architecturally distinct from finance,
bot, and chat: file has no database tables at all. What it owns is the
authenticated **file control plane** over infrastructure the host still runs.
...
(`modules/chat/ui/message-bubble.tsx`) and then published from an isolated
...
rollback targets post-file-cutover. `modules/note` (todo 3071, unpublished
at the time of this cut) still asserts the built-in `/api/file/*` paths in
its own tests and will need its own retarget before its first publish.
...
`250d5b8`, `ui=e859bebc25dc` / `api=628244cb50ae`. Roll forward is the only
recovery path for this module until a later twin is intentionally published.
