...
| 3272 | Hardened Monitor v4 response and saved-state boundaries after partial or stale summary data could reach `series.map` during overview rendering. Summary, route, and event payloads now validate required fields and collection members all-or-nothing; valid empty arrays preserve empty states, while malformed payloads render the explicit error state and remain eligible for SWR revalidation. Local source commit `2844c95`; module publish awaits explicit approval. | - | - | - | `pages/review-3272-monitor-payload-boundaries.md` | implemented and reviewed; publish pending approval |
