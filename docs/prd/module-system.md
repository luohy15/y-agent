...
**v9**, `ui=0446b41ab198` / `api=628244cb50ae`). **v1–v4 all hang on ordinary-file
...
restored the full audited pre-cut viewer behavior, and v9 applies Roy's tidier
toolbar direction: no Search or Import Note buttons, with Raw/Preview and History
as compact icons while relative path + adjacent Copy and the consolidated
Markdown/HTML/PDF download control remain. It is architecturally distinct from
...
| 3084 | Restored the ordinary-file viewer behavior dropped in the todo 3068 cut, then converged its toolbar to Roy's explicitly tidier target. File v7 restored the first affordance pass; v8 restored the full audited pre-cut behavior; active v9 (`ui=0446b41ab198…` / `api=628244cb50ae…`, 129222-byte UI bundle) removes the Search and Import Note toolbar entries and renders Raw/Preview plus History as compact accessible icons, while retaining canonical relative path + immediately adjacent Copy, the single Markdown/HTML/PDF download menu, Markdown navigation/front matter, mounted per-tab state, shortcuts, and the remaining viewer parity fixes. Search stays reachable through the host `file.search` intent. Rollback is `y module rollback file` to v8. Module source commits `dd011ae` and `afc65ad`; no host/contract change or database migration. Known host-seam limitation: shortcuts forwarded from a sandboxed HTML preview still double-fire because the host replays them at `window`; no module-local listener ordering can prevent it. | - | `pages/plan-3084-file-viewer-affordances.md` | - | `pages/review-3084-file-viewer-affordances.md` | shipped; runtime UI verification pending |
