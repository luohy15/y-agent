...
| 3084 | Restored the complete ordinary-file viewer behavior dropped in the todo 3068 cut. The initial file v7 pass restored toolbar copy-path/icon styling, image zoom/scrolling, tab affordances, and search-dialog behavior. A full pre-cut audit then shipped file v8 (`ui=7b3393a742dd…` / `api=628244cb50ae…`, 129056-byte UI bundle): canonical relative path display with Copy immediately adjacent, one Markdown/HTML/PDF download menu, Markdown local-link/front-matter/external-link behavior, empty-Markdown raw mode, adjacent close selection, preview double-click pinning, mounted per-tab DOM preserving scroll/cursor/undo, internal tab shortcuts, iframe/save styling, and active-context search. Rollback is `y module rollback file` to v7. Module source commit `dd011ae`; no host/contract change or database migration. Known host-seam limitation: shortcuts forwarded from a sandboxed HTML preview still double-fire because the host replays them at `window`; no module-local listener ordering can prevent it. | - | `pages/plan-3084-file-viewer-affordances.md` | - | `pages/review-3084-file-viewer-affordances.md` | shipped; runtime UI verification pending |
