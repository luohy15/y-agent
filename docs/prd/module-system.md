...
Todo Detail (todo 3179) is the next browser-contract leaf after `CodeEditor`.
The authenticated todo detail / public-trace presentation stays one physical
host implementation (`TraceView` + waterfall + share + todo detail). Browser
contract **v9 → v10** exports that leaf on `@y/host` so the Todo module's
`detail` surface can mount it for an in-place Finance-style drill-down, rather
than copying the presentation into y-module or keeping a second path behind the
...
| 3179 | Todo Detail in-place viewer reuses one host-owned `TraceView` leaf: browser contract **v9 → v10** exports `TraceView` and its public prop/data types on `@y/host`, adds narrow adapters (`link.open`, `calendar.focusDate`, `trace.clearRoute`), and hardens required-todo / retryable-error / malformed-payload states without changing injected public mode. Staged after host deploy A: publish the Todo module that depends on v10, verify parity, then retire authenticated `trace.md` entry points. Feature home: `code/y-agent/docs/prd/todo-detail-view.md`. | - | `pages/plan-3179-todo-detail-view.md` | - | `pages/review-3179-host-traceview-export.md` | H1 approved; remaining sub-tasks planned |
...
| 3272 | Hardened Monitor v4 response and saved-state boundaries after partial or stale summary data could reach `series.map` during overview rendering. Summary, route, and event payloads now validate required fields and collection members all-or-nothing; valid empty arrays preserve empty states, while malformed payloads render the explicit error state and remain eligible for SWR revalidation. Published as Monitor v5 (`ui=2415ae4cba33…`, `api=af646a43a3bb…`) from y-module `2844c95`; rollback target is v4 via `y module rollback monitor`. | - | - | - | `pages/review-3272-monitor-payload-boundaries.md` | shipped; runtime UI verification pending |
