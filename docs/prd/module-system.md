...
in backend contract v8; it neither reads the database directly nor duplicates
normalization or resolver behavior. The host retains the projection table,
normalization, carrier synchronization and cleanup, resolver hydration, every
carrier's exact-tag filters, and the `tag.open` navigation adapter. Disabling or
deleting the module removes tag browsing and command presentation only. Carrier
authoring, projection maintenance, and filters keep working in the kernel.
`y module rollback tag` is a no-op while v1 is the only version; recovery is
`y module disable tag` or `y module delete tag`. Instance inventory:
