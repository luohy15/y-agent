...
| PRD | Scope |
|-----|-------|
| [bot-dispatch-tier-routing](bot-dispatch-tier-routing.md) | Routes each dispatched session to a bot by capability tier: bots carry a tier as data, dispatch targeting is a bot name or a tier (unspecified defaults to tier2, never a skill-derived tier), explicit bot/backend pins bypass routing, and a documented policy governs which work deserves the strong tiers. |
