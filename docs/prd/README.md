...
| [module-system](module-system.md) | Owns the module as a versioned unit shipped outside the deploy pipeline: one VM directory holding a domain's API routes, CLI commands, logic, and UI, loaded in-process by the API behind a per-version sub-app and by the CLI from local source, with one atomic version spanning the published API and UI halves; absorbs `y ui` into `y module` and takes finance as the reference migration. |
