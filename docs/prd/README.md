...
| [ui-dynamic-artifacts](ui-dynamic-artifacts.md) | Owns per-user UI that ships outside the application bundle: VM-hosted TSX source built by `y ui publish` into immutable, content-hashed ES modules, loaded at runtime as real React panels against a versioned host SDK, with pointer-move publish/rollback and per-panel failure isolation so optional screens (Finance) need no app deploy. |
