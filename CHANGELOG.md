...
- **Provider subscription usage cards (2872)**: `y usage limits [--json] [--refresh]` reads Claude/Codex/Grok subscription usage directly from each provider (Claude via a revived `/usage` TUI scrape, Codex via `GET /backend-api/codex/usage`, Grok via `GET /v1/billing?format=credits`), backed by a locked credential store at `$Y_AGENT_HOME/.credentials/provider-usage.json` with per-provider token refresh and `y usage login <openai|xai>` / `y usage credentials`. `/api/usage/limits` now SSHes into the VM CLI instead of reading claude-relay-service, with `billing_period` as a first-class window kind and a per-user TTL memo that serves the last good snapshot as stale rather than blanking the card on failure. The web BotViewer renders one card per provider with per-provider window kinds (Grok's billing period vs Claude/Codex's 5h/1w windows), an actionable `reauth_required` state with human-readable copy, and a manual-retry button wired to a one-shot refresh so it can't replay a memoized SWR failure.
- **Live price-series endpoint (finance)**: `/api/finance/price-series` and `y finance price-series` pass through live per-range quote data (`1w`/`1m`/`3m`/`ytd`/`1y`/`5y`/`all`) with matching granularity, independent of the beancount snapshot.
- **`y ui delete` (2941)**: hard-deletes a dynamic UI artifact and all of its version rows, best-effort cleans up the stored bundle bytes, and leaves the VM authoring source (`.tsx`/`.json`/parts directory) untouched.
- **Context handoff reminder (2951)**: inbound user messages get an appended reminder once a chat's used tokens exceed `min(50% of context window, 200k tokens)`, wired at all five existing-chat write sites (API send + notify, Telegram routed / DM steer / DM append). Wording states the session's actual usage percentage and token count, chat-creation sites are skipped, and `english_correction` strips the trailing reminder block so an injected nag is never scored as the user's own prose.
- **Per-version UI artifact descriptions (2991)**: `y ui publish -d/--desc` sets a write-once `description` on `ui_artifact_version`, auto-tagged with `[<Y_TRACE_ID>] ` by the CLI.
- **Retained artifact intent channel (2979, contract v3)**: `@y/host` gains `useArtifactIntent`/`openArtifactDetail` so built-in host code (a trace click, a tag drill-down) can navigate into a dynamic UI artifact across the module boundary; Calendar is migrated onto the dynamic `calendar` artifact using it (agenda panel + week grid, drag/resize, event modal, all three inbound navigation paths retargeted).
- **Bot usage over-time legend + click-to-filter (2981)**: the "Tokens over time" chart gains a top-5-plus-`Other` legend row with color swatches; clicking an entry filters the chart to that model (click again to clear), with `aria-pressed` toggle state and a caption disclosing the filtered model and the `Other` bucket's scope. The table below the chart stays unfiltered.

### Changed
- **Re-bot an existing chat mid-conversation (2930)**: naming a different bot on a chat that already has one no longer logs "ignoring bot_name change": the chat switches to it. The new bot's name and tier are persisted (they leave the write-once identity set), `external_id` is kept, and the Claude Code session resumes under the new bot's `--model` and relay credentials, so a live conversation can move up to a stronger model or down to a cheaper one without losing its context. The chat's `backend` is still fixed: a bot on a different backend is refused with a log and the chat keeps its current bot, because the inline `perplexity` / `openai` backends have no session to resume. A change is only honored when the requested name actually resolves to a runnable bot; a mistyped, disabled, or wrong-tier name leaves the chat where it is instead of landing it on the tier2 fallback, so a stale bot name cannot silently downgrade a live conversation. Surfaced through `y chat --chat-id <id> --bot <name>` and the web bot picker, which now acts on the selected chat instead of only on new ones.
...
- **Bot usage daily-tokens heatmap rebucketed to absolute thresholds (2982)**: bucket boundaries move from a share of the window's max value to absolute per-metric thresholds (100M tokens / $100 cost / 1,000 requests, five steps to a `5x` ceiling), with days above the ceiling on a linear ramp interpolated to the window max; colors move from alpha-over-card steps to opaque greens sampled off one gradient per theme mode, since alpha steps flip direction with the theme and cannot continue past `alpha=1`. A quiet window no longer self-normalizes to a full-scale spread.
- **Bot usage dashboard caption cleanup (2988)**: removed the small explanatory captions beside dashboard titles across Subscription limits, the daily heatmap, the over-time chart, and the history table.
...
- **English correction dispatch-prefix filtering (2871)**: the leading meta-prefix skip is generalized from `[trace:...]` / `[routine:...]` only to any bracketed block carrying dispatch/routine meta keys (`trace:` / `from:` / `to:` / `from_chat:` / `to_chat:` / `routine:`, each immediately followed by a non-space character), so no-trace-id fan-out dispatches relayed as `[from:... from_chat:... to_chat:...]` are skipped; authored prose that opens with a plain bracket (e.g. `[draft] ...`) or with a meta-looking key followed by a space (e.g. `[note to: myself] ...`) is still scanned. Pure key=value machine-payload bodies (every whitespace token contains `=`) are also rejected as non-prose, while prose that merely contains `=` is kept.
...
- **Usage relative time tokens (2872)**: the write path stamped `usage_date` in the configured timezone while the read path resolved relative tokens (`today`/`ytd`/`day-7`) against process-local UTC, so between 00:00–08:00 local the Live usage tab (and `/api/finance/*` / `y finance --time ...`, which share the parser) requested the wrong day. A shared `local_today()` helper now backs both paths.
- **Dynamic UI artifact tabs (2943)**: artifact (`ui:`) tabs now participate in the persisted open-tabs/order state instead of always reopening last after a reload; the `/ui/<slug>` route is retired (redirects to `/`) in favor of root-level tab restore.
- **Dynamic UI artifact SDK gaps**: the artifact host type stub now declares a JSX namespace so artifact source type-checks, and the SDK tsconfig covers multi-file `ui/<slug>/` parts directories, not just the top-level entry.
- **Chat session-resume authority (2930)**: a refused `claude -p -r` resume is now detected from the error result event instead of the no-result branch, and the persisted `external_id` column (not its `json_content` copy) is the sole authority for which session handle to drop, so only a handle the CLI has affirmatively refused is cleared instead of any handle absent a result.
- **Bot usage daily-tokens heatmap empty-cell color (2980)**: the no-usage level was the same color as the card border and table header, so empty cells vanished under solarized-light; level 0 is now a neutral gray derived per theme, GitHub-style, computed under `useThemeColors()` so it repaints on a live theme toggle instead of freezing at import.
- **Claude Code context window for unrecognized custom models (2993)**: a chat on a plain (non-`[1m]`-suffixed) custom model id reported a 200K context window because Claude Code treats an unknown model id as a 200K variant; every Claude Code subprocess now launches with `CLAUDE_CODE_MAX_CONTEXT_TOKENS=1000000` so unrecognized custom model ids report and use a 1M window without needing the model-string suffix, while known Claude models keep their registered windows. Supersedes the earlier per-bot `[1m]`-suffix workaround (2989) as the active mechanism; the handoff reminder threshold stays capped at 200K used tokens.
...
- **Built-in Finance web surface**: `FinanceViewer`, `FinancePanel`, `lib/finance*.ts`, and the ActivityBar Finance entry are deleted now that the dynamic `finance` UI artifact has been verified live; Finance in the web UI depends entirely on the published artifact, and `y ui rollback finance` is the only recovery path for a broken one.
- **Built-in Bot web surface (2970)**: `BotList`, `BotViewer`, and their vitest coverage are deleted now that the dynamic `bot` UI artifact is the only bot surface (config + usage); `y ui rollback bot` is the recovery path for a broken artifact.
- **Built-in Calendar web surface (2979)**: `CalendarViewer` and `ScheduleList` are deleted now that Calendar runs entirely on the dynamic `calendar` UI artifact; `y ui rollback calendar` is the recovery path for a broken artifact.
