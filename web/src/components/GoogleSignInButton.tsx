...
export default function GoogleSignInButton({ gsiReady, className }: GoogleSignInButtonProps) {
  if (isPreview) {
    ...
    return (
      <a
        href={loginUrl}
        className={className ?? "px-5 py-2.5 bg-sol-base02 border border-sol-base01 text-sol-base1 rounded text-sm font-semibold cursor-pointer hover:bg-sol-base01 hover:text-sol-base2"}
      >
        Sign in with Google
      </a>
    );
  ...
  return (
    <div className="relative inline-flex items-center justify-center">
      <span className={className ?? "px-5 py-2.5 bg-sol-base02 border border-sol-base021 text-sol-base1 rounded text-sm font-semibold pointer-events-none"}>
        Sign in with Google
      </span>
