...
interface SharedNoteProps {
  /** Note share id (the `/n/<shareId>` segment). */
  shareId: string;
  /** When provided, renders a "← Back to trace" button and is called on click. */
  onBack?: () => void;
}

/**
 * Reusable shared-note body: fetch + password + loading/error + tags + markdown
 * article + TOC. Owns no outer sizing (no min-h-screen): the parent provides a
 * full-height scroll container (standalone wrapper or in-trace overlay), so the
 * sticky header works under either scroll context.
 */
export default function SharedNote({ shareId, onBack }: SharedNoteProps) {
  ...
  const copyLink = () => {
    navigator.clipboard.writeText(`${window.location.origin}/n/${shareId}`);
    setShareLabel("copied!");
    setTimeout(() => setShareLabel("share"), 1500);
  };

  if (loading) {
    return <div className="flex-1 flex items-center justify-center text-sm text-sol-base1">Loading shared note...</div>;
  }
  ...
  if (error || !data) {
    return <div className="flex-1 flex items-center justify-center text-sm text-sol-red">{error || "Shared note not found"}</div>;
  }
        ...
        <div className="max-w-4xl mx-auto flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            {onBack && (
              <button
                onClick={onBack}
                className="shrink-0 font-mono cursor-pointer px-2 py-1 rounded text-xs font-semibold bg-sol-base02 text-sol-base01 hover:text-sol-base0"
              >
                ← Back to trace
              </button>
            )}
