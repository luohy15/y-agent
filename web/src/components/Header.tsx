...
export default function Header({ email, isLoggedIn, gsiReady, onLogout, onToggleChatList, chatListOpen, onToggleActivityBar, activityBarOpen, onClickLogo, onOpenFileSearch }: HeaderProps) {
  ...
  return (
    <header className="px-4 md:px-6 py-4 border-b border-sol-base02 shrink-0 flex items-center justify-between">
      <div className="flex items-center gap-2">
        ...
        {/* Mobile-only: File search */}
        {isLoggedIn && onOpenFileSearch && (
          <button
            onClick={onOpenFileSearch}
            className="md:hidden h-8 flex items-center gap-1.5 px-2 text-sm cursor-pointer rounded hover:bg-sol-base02 text-sol-base01 hover:text-sol-base1"
