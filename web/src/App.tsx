...
export default function App() {
  ...
  const [selectedTickerSymbol, setSelectedTickerSymbol] = useState<string | null>(() => localStorage.getItem("selectedTickerSymbol") || null);
  ...
  // Calendar navigation target from TraceView event clicks; fresh object identity
  // per click so CalendarViewer re-jumps even when the date is unchanged.
  const [calendarFocus, setCalendarFocus] = useState<{ date: string } | null>(null);
  ...
  const [rightPanel, setRightPanel] = useState<RightPanel>(() => {
    ...
    return saved === "chats" || saved === "notes" || saved === "links" || saved === "files" || saved === "diff" ? saved : "chats";
  });
  const [chatContextPanel, setChatContextPanel] = useState<ChatContextPanel>(() => {
    ...
    return saved === "links" || saved === "files" || saved === "diff" ? saved : "notes";
  });
  ...
  useEffect(() => { if (selectedTickerSymbol) localStorage.setItem("selectedTickerSymbol", selectedTickerSymbol); else localStorage.removeItem("selectedTickerSymbol"); }, [selectedTickerSymbol]);
  ...
  const renderChatContextPanel = (mobile = false) => (
    <div className="h-full min-h-0 flex flex-col">
      <div className="flex items-center gap-1 px-2 py-1 border-b border-sol-base02 shrink-0">
        ...
        <button onClick={() => setChatContextPanel("notes")} className={chatContextBtnClass(chatContextPanel === "notes")}>Note</button>
        <button onClick={() => setChatContextPanel("links")} className={chatContextBtnClass(chatContextPanel === "links")}>Link</button>
        <button onClick={() => setChatContextPanel("files")} className={chatContextBtnClass(chatContextPanel === "files")}>File</button>
        <button onClick={() => setChatContextPanel("diff")} className={chatContextBtnClass(chatContextPanel === "diff")}>Diff</button>
      </div>
      <div className="flex-1 min-h-0 overflow-hidden">
        {chatContextPanel === "notes" ? (
          ...
          <FileTree isLoggedIn={auth.isLoggedIn} onSelectFile={mobile ? (path) => { handlePreviewFile(path); setChatListOpen(false); } : handlePreviewFile} onDeleteFile={handleCloseFile} onRenameFile={handleRenamedFile} vmName={selectedVM} workDir={effectiveWorkDir} refreshKey={chatContextRefreshKey} />
        ) : (
            ...
            <div className="flex-1 min-w-0 min-h-0 flex flex-col overflow-hidden relative">
              ...
              <div className={`absolute inset-0 ${chatHide ? "" : "hidden"}`}>
                <ErrorBoundary label="Panel">
                  <FileViewer openFiles={openFiles} activeFile={activeFile} onSelectFile={setActiveFile} onCloseFile={handleCloseFile} onReorderFiles={setOpenFiles} vmName={selectedVM} workDir={effectiveWorkDir} defaultWorkDir={defaultWorkDir} diffFiles={diffFiles} artifactTabs={artifactTabs} uiArtifacts={mountedUiArtifacts} onUiArtifactRolledBack={() => { void mutateUiArtifacts(); }} isLoggedIn={auth.isLoggedIn} selectedTraceId={selectedTraceId} selectedLinkId={selectedLinkId} selectedLinkLinkId={selectedLinkLinkId} selectedLinkContentKey={selectedLinkContentKey} selectedEntityId={selectedEntityId} selectedTickerSymbol={selectedTickerSymbol} onSelectTicker={(symbol: string) => { setSelectedTickerSymbol(symbol); handleOpenFile("ticker.md"); }} selectedCorrectionId={selectedCorrectionId} selectedThreadId={selectedThreadId} selectedThreadAccount={selectedThreadAccount} selectedFeedId={selectedFeedId} selectedFeedLabel={selectedFeedLabel} onClearFeed={handleClearFeed} onSelectChat={(id) => { setSelectedChatId(id); setChatListOpen(false); setChatHide(false); }} onSelectTrace={(traceId) => { requestSelectTraceId(traceId); handleOpenFile("trace.md"); }} onSelectCalendarEvent={(startTime) => { setCalendarFocus({ date: startTime }); handleOpenFile("calendar.md"); }} calendarFocus={calendarFocus} onPreviewLink={(activityId) => { setSelectedLinkId(activityId); setSelectedLinkLinkId(null); handleOpenFile("link.md"); }} onPreviewLinkFull={(activityId, contentKey) => { setSelectedLinkId(activityId); setSelectedLinkLinkId(null); setSelectedLinkContentKey(contentKey); handleOpenFile("link.md"); }} onExternalLinkClick={handleExternalLinkClick} previewFile={previewFile} onPinFile={handlePinFile} onPreviewFile={handlePreviewFile} pendingLines={pendingLines} onConsumeLine={handleConsumeLine} onChatListRefresh={() => setChatListRefreshKey((k) => k + 1)} onTraceTodoDirtyChange={setTraceTodoDirty} />
                </ErrorBoundary>
              </div>
          ...
          {!rightPanelCollapsed && (
            ...
            >
              ...
              <div className="flex items-center gap-1 px-2 py-0.5 border-b border-sol-base02 shrink-0">
                <button onClick={() => setRightPanel("chats")} className={rightPanelBtnClass(rightPanel === "chats")} title="Chat">
                ...
                <button onClick={() => setRightPanel("notes")} className={rightPanelBtnClass(rightPanel === "notes")} title="Note">
                ...
                <button onClick={() => setRightPanel("diff")} className={rightPanelBtnClass(rightPanel === "diff")} title="Diff">
              ...
              <div className="flex-1 min-h-0 overflow-hidden">
                <ErrorBoundary label="Panel">
                  {rightPanel === "chats" ? (
                    ...
                    <FileTree isLoggedIn={auth.isLoggedIn} onSelectFile={handlePreviewFile} onDeleteFile={handleCloseFile} onRenameFile={handleRenamedFile} vmName={selectedVM} workDir={effectiveWorkDir} refreshKey={chatContextRefreshKey} />
                  ) : (
            ...
            <div className="flex-1 min-h-0 overflow-hidden">
              <ErrorBoundary label="Panel">
                {rightPanel === "chats" ? (
                  ...
                  <FileTree isLoggedIn={auth.isLoggedIn} onSelectFile={(path) => { handlePreviewFile(path); setChatListOpen(false); }} onDeleteFile={handleCloseFile} onRenameFile={handleRenamedFile} vmName={selectedVM} workDir={effectiveWorkDir} refreshKey={chatContextRefreshKey} />
                ) : (
