...
import { fetchPublicDemo, type PublicDemoRef } from "./lookup";
import { DEMO_ROUTES, demoPath, demoRouteFor, type DemoRoute } from "./routes";

type LookupState =
  | { status: "loading" }
  | { status: "ready"; demo: PublicDemoRef }
  | { status: "unavailable" };
...
function DemoBody({ route }: { route: DemoRoute }) {
  ...
  useEffect(() => {
    ...
    fetchPublicDemo(route.key)
      .then((demo) => {
        if (!cancelled) setState({ status: "ready", demo });
      })
      .catch((err: unknown) => {
        if (cancelled) return;
  ...
  return <DemoMount slug={state.demo.slug} version={state.demo} />;
}
