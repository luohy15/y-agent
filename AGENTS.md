...
- **Hot-loadable modules** — a user-owned, versioned domain with optional API/UI/
  ...
  keeps the `@y/host` browser contract (**v18**), integrity check, and error
  ...
  Backend host contract (`agent.module_host`) is **v18** (todo 3627 adds
  owner-bound no-wake VM execution; File requires host v18 before publication). The tag module owns
...
- `module_host.py` — backend host contract for modules (`BACKEND_CONTRACT_VERSION = 18`:
...
- Frontend: TypeScript strict, TailwindCSS utility classes, Solarized light/dark
  ...
  visual reference `docs/prd/design-language.html`.
