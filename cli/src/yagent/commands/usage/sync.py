...
@click.command("sync")
@click.option("--source", type=click.Choice(["crs", "openrouter"]), default=None, help="Limit to one source (default: all)")
@click.option("--days", type=int, default=None, help="OpenRouter: keep only the last N days of /activity rows")
...
def sync(source: str | None, days: int | None, user_id: int | None, as_json: bool):
    """Pull daily LLM token/cost usage into model_usage_daily (idempotent upsert)."""
    target_user_id = user_id or get_cli_user_id()
    result = usage_service.sync(target_user_id, source=source, days=days)
    if as_json:
