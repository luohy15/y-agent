...
`/usage` overlay, and reports the limit windows (current session / current week
all-models / an optional third weekly window under whatever model/promo label
the overlay currently shows -- see `agent.claude_usage.parse_usage_pane`) as a
friendly table or, with `--json`, the raw envelope.
"""
...
def usage(user_id: int | None, vm_name: str | None, work_dir: str | None, as_json: bool):
    ...
    envelope = {
        "data": {
            ...
            "week_extra": result.get("week_extra"),
        },
    ...
    extra = envelope["data"].get("week_extra")
    if extra:
        label = f"Current week ({extra['label']})"
        pct = f"{extra['percent']}%" if extra.get("percent") is not None else "-"
        click.echo(f"{label:<28} {pct:>6}  {extra.get('reset') or '-'}")
