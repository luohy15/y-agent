...
from ._helpers import echo_json, json_option, rows_envelope
from ._render import render_prices
...
@json_option
def prices(symbol: str | None, time: str, limit: int, as_json: bool):
    """Read DB-backed finance prices (table by default; --json for the raw envelope)."""
    ...
    envelope = rows_envelope(rows)
    if as_json:
        echo_json(envelope)
    else:
        render_prices(envelope)
